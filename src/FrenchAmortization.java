...
public class FrenchAmortization {
	...
	public void calculateFee() {
		...
		for(int i = 0; i <= requestDTO.getFee(); i++) { 
			...
			toPayPerFee = requestDTO.getLoan() * (Math.pow(1+requestDTO.getInterest()/100, requestDTO.getFee())*requestDTO.getInterest()/100)/(Math.pow(1+requestDTO.getInterest()/100, requestDTO.getFee())-1);
			...
			System.out.printf("Cuota: "+i+": %f, Interes: %f, Cuota de amortizacion: %f, Capital: %f", toPayPerFee+i, feeInterest, amortizationFee, pendingCapital);
